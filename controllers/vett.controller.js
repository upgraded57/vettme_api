...
} = require("../functions/createRecordAndSendResponse");
...
const verifyPersonnel = async (req, res) => {
  ...
  // Driver's licence verification process
  ...
  // Voter's id verification process
  if (type === "voters_id") {
    // Throw error if licence number is not provided
    if (!data.vin) {
      throw new BadRequestException(
  ...
  // Throw error if specified type is not valid
  else {
    throw new BadRequestException(
      "Unknown verification type provided",
      verificationErrors.INVALID_VERIFICATION_DATA
    );
  }
};
